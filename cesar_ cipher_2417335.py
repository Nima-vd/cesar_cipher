...
def message_or_file():
    enter_message_result = enter_message()
    
    # Ensure that the result always has four elements
    if len(enter_message_result) == 3:
        return enter_message_result[0], enter_message_result[1], None, enter_message_result[2]
    else:
        return enter_message_result
...
def main():
    ...
    while True:
        ...
        if fileName is not None:
            ...
            write_messages(
                [encrypt(message, shift) if mode == 'e'
                 else decrypt(message, shift) for message in messages])
